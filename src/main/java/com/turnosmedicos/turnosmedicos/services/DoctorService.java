...
public class DoctorService {
    ...
    public Doctor getDoctor(String token, Long id) {
        if(authService.validationToken(token)) {
            return doctorRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("The user with this id: " + id + "is not found"));
        } throw new UnauthorizedException("Invalid token");
    }
}
